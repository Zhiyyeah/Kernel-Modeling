...
def main() -> None:
    ...
    model = DegradationModel(in_ch=5, mid_ch=32, ks=(7, 5, 3, 1, 1, 1), scale_gain=0.1, noise_init=0.01, noise_max=0.2)
    ...
    for t in range(iters):
        ...
        if (t + 1) % mini_log_every == 0:
            print(f"Iter {t+1}/{iters} | D: {loss_D.item():.4f} | G_adv: {loss_G_adv.item():.4f} | Noise: {loss_noise.item():.4f} | sigma_mean: {sigma.mean().item():.4f}")
        if (t + 1) % log_every == 0:
            extra = generator_weight_stats(model.generator) if verbose else ''
            print(f"[LOG] Iter {t+1}/{iters} | D: {loss_D.item():.4f} | G_adv: {loss_G_adv.item():.4f} | Reg: {loss_reg.item():.4f} | Noise: {loss_noise.item():.4f} {extra}")
        ...
        if (t + 1) % kernel_log_every == 0:
            ...
            if verbose:
                ...
                print(f"  [Bands] {band_max} | sigma_mean={sigma.mean().item():.4f}")
