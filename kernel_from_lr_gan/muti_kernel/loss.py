...
def noise_reg_loss(sigma: torch.Tensor, target: Union[float, torch.Tensor] = 0.01, mode: str = 'l2') -> torch.Tensor:
    """
    噪声强度正则：约束 sigma 不要过大，避免把高频细节全部吸收到噪声里。
    sigma: [C]
    target: 标量或 [C]，用于分波段约束
    mode: 'l1' 或 'l2'
    """
    target_tensor = torch.as_tensor(data=target, device=sigma.device, dtype=sigma.dtype)
    if mode == 'l1':
        return torch.mean(torch.abs(sigma - target_tensor))
    return torch.mean((sigma - target_tensor) ** 2)
